  ...
  $.fn.formwizard = function(wizardSettings, validationSettings, formOptions){
	...
	function initialize(){
		...
		form[0].settings = $.extend({
			...
			back : ":reset",
			next : ":submit",
			submitButtonClass : 'submit_button',
			textSubmit : 'Submit',
			textNext : 'Next',
			textBack : 'Back',
			afterNext: undefined,
	...
	function enableNavigation(){
		if(form[0].isLastStep){
			form[0].nextButton.addClass(form[0].settings.submitButtonClass);
			form[0].nextButton.val(form[0].settings.textSubmit);
		}else{
			form[0].nextButton.removeClass(form[0].settings.submitButtonClass);
